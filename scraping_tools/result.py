...
try:
    ...
    query = "SELECT * FROM core_dolloarcategory"
    ## getting records from the table
    cursor.execute(query)

    ## fetching all records from the 'cursor' object
    dolloar_categories = cursor.fetchall()
    ...
    for i in range(len(department)):
        ...
        while 1:
            ...
            sub_categories = driver.find_element_by_class_name('col-lg-2').find_elements_by_tag_name('ul')[1].find_elements_by_tag_name('li')

            sub_categories_url = driver.current_url
            return_count = len(sub_categories)

            for categories_index in range(return_count):
                #click the sub categories
                categories_name = driver.find_element_by_class_name('col-lg-2').find_elements_by_tag_name('ul')[1].find_elements_by_tag_name('li')[categories_index].find_element_by_tag_name('a').text
                categories_id = ""

                for dolloar_category in dolloar_categories:
                    if categories_name==dolloar_category[2]:
                        categories_id = dolloar_category[0]

                driver.find_element_by_class_name('col-lg-2').find_elements_by_tag_name('ul')[1].find_elements_by_tag_name('li')[categories_index].click()
                ...
                for products_index in range(len(products)-1):
                    ...
                    try:# wait for new window
                        ...
                        print('network error')
                if driver.find_element_by_class_name('pagination-dropdown').find_elements_by_class_name('form-group')[2].find_element_by_tag_name('ul').find_element_by_tag_name('li').get_attribute('class') != 'next  disabled':
                    driver.find_element_by_class_name('pagination-dropdown').find_elements_by_class_name('form-group')[2].find_element_by_tag_name('ul').find_element_by_tag_name('li').find_element_by_tag_name('a').click()
                else:
                    if categories_index==return_count-1:
                        break
                    else: 
                        driver.get(sub_categories_url)
