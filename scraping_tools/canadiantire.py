...
try:
    ...
    for product_index in range(len(product_url_list)):
        ...
        try:# wait for new window
            WebDriverWait(driver, 60).until(EC.visibility_of_element_located((By.CLASS_NAME,"pdp-product-image-and-buy-box")))
            time.sleep(10)
        except TimeoutException:
            continue
        try:
        ...
        try:# wait for new window
            WebDriverWait(driver, 60).until(EC.visibility_of_element_located((By.CLASS_NAME,"recently-viewed-product-tiles__list")))
            related_list = driver.find_element_by_class_name('recently-viewed-product-tiles__list').find_element_by_class_name('owl-stage-outer').find_element_by_class_name('owl-stage').find_elements_by_class_name('owl-item')
            for related_index in range(len(related_list)):
                related_products.append(driver.find_element_by_class_name('recently-viewed-product-tiles__list').find_element_by_class_name('owl-stage-outer').find_element_by_class_name('owl-stage').find_elements_by_class_name('owl-item')[related_index].find_element_by_class_name('product-tile--light').get_attribute('data-productid'))
        except TimeoutException:
            related_products = ''
            print('No related_list')
        try:
            product_current_price = driver.find_element_by_class_name('price__total-value').get_attribute('innerHTML')
            product_current_price = float(product_current_price.split('$')[1])
        except NoSuchElementException:
            product_current_price = ''
            print('No product_current_price')
        try: 
            produt_last_price = driver.find_element_by_class_name('price__reg-value').get_attribute('innerHTML')
            produt_last_price = float(produt_last_price.split('$')[1])
        except NoSuchElementException:
            produt_last_price = ''
        ...
        try:
            ...
            for feature_index in range(len(feature_list)):
                product_feature.append(driver.find_element_by_class_name('pdp-details-features__items').find_element_by_tag_name('ul').find_elements_by_tag_name('li')[feature_index].get_attribute('innerHTML'))
        except NoSuchElementException:
            product_feature = ''
            print('No product_feature')

        print(produt_last_price, product_current_price, product_feature)
        ...
        if product_record == None:
            product_data_list = []
            product_data_list.append((product_id,product_name,str(product_images), str(product_category), str(produt_last_price), str(product_current_price), str(product_store), str(product_feature), str(product_specification), str(related_products)))
            ...
            update_data_array = ()
            update_data_array = (product_name,str(product_images), str(product_category), str(produt_last_price), str(product_current_price), str(product_store), str(product_feature), str(product_specification), str(related_products), product_id)
