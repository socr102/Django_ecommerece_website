...
class HomeView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        categories = dolloarCategory.objects.all()
...
def subcategory(request):
    ...
    products = []
    if subcategoies:
        return HttpResponse(serializers.serialize('json', subcategoies), content_type="text/json-comment-filtered")
        
    else:
        products =  dolloarItem.objects.filter(category_id=category_id)
        return HttpResponse(serializers.serialize('json', products), content_type="text/json-comment-filtered")

@csrf_exempt
def back_tree(request):
    category_id = request.POST.get('category_id')
    shop_id = request.POST.get('shop_id')
